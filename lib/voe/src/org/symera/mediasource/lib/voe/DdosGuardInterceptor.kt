package org.symera.mediasource.lib.voe
...
class DdosGuardInterceptor(private val client: OkHttpClient) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val oldCookie = if (!cookies.isNullOrEmpty()) cookies.split(";").mapNotNull { Cookie.parse(originalRequest.url, it) } else emptyList()
    ...
    fun getNewCookie(url: HttpUrl): Cookie? {
        ...
        val oldCookie = if (!cookies.isNullOrEmpty()) cookies.split(";").mapNotNull { Cookie.parse(url, it) } else emptyList()
        val ddg2Cookie = oldCookie.firstOrNull { it.name == "__ddg2_" }
        if (!ddg2Cookie?.value.isNullOrEmpty()) return ddg2Cookie
        ...
        return client.newCall(GET(checkUrl)).execute().header("set-cookie")?.let { Cookie.parse(url, it) }
    }
