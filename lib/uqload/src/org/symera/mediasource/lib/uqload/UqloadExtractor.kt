...
class UqloadExtractor(private val client: OkHttpClient) {
    suspend fun streamsFromUrl(url: String, prefix: String = ""): List<SStream> {
        ...
        val script = doc.selectFirst("script:containsData(eval)")?.data() ?: return emptyList()
        val unpacked = autoUnpacker(script) ?: return emptyList()
        val videoUrl = VIDEO_URL.find(unpacked)?.groupValues?.get(1) ?: return emptyList()
    ...
    companion object {
        ...
        private val VIDEO_URL = Regex("""(?:file|sources)\s*:\s*(?:\[\s*)?(?:\{\s*)?[\"'](https?://[^\"']+\.m3u8[^\"']*)""")
    }
}
